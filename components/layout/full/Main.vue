...
<template>
    <!------Sidebar-------->
    <v-navigation-drawer left elevation="0" app class="leftSidebar bg-warning" v-model="sDrawer">
        <!---Logo part -->
        <div class="pa-5">
            <LayoutFullLogo />
        </div>
        ...
        <div>
                <ul v-auto-animate class="pt-2 m-1">
                    <li v-for="(item, i) in sidebarMenu" :key="i" @click="activeLink(item)"
                        v-bind:class="item.title == selectedMenu && !item.expanded ? 'bg-gray-400 hover:bg-gray-300  ' : 'hover:bg-gray-400'"
